...
import { createTestUser, registerUser, loginUser, logoutUser, clearAuth } from './helpers';
...
test.describe('Session Management', () => {
  ...
  test('should clear sensitive data on logout', async ({ page }) => {
    ...
    expect(afterLogout.local.length).toBeLessThanOrEqual(beforeLogout.local.length);
  ...
  test('should handle session hijacking prevention', async ({ browser }) => {
    ...
    const isProtected = await page2.url() === '/' || 

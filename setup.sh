...
echo "4. Set up frontend and generate SDK types"
echo ""
echo "   The project uses OpenAPI-first architecture with automatic type generation."
echo ""
echo "   a. Install frontend dependencies:"
echo "      cd demo-app && pnpm install"
echo ""
echo "   b. Start backend dev server (generates OpenAPI spec):"
echo "      cd .. && pnpm run dev"
echo ""
echo "   c. In a new terminal, generate SDK types:"
echo "      cd demo-app && pnpm run generate:sdk"
echo ""
echo "   Note: The OpenAPI spec is auto-generated at /openapi.json"
echo "   You can fetch it with: curl http://localhost:8787/openapi.json > openapi.json"
echo ""
echo "5. AWS SES Production Access (Required for sending to any email)"
echo ""
...
echo ""
echo "   $ ./scripts/testing/test-registration-backend.sh"
echo ""

print_header "Architecture Overview"

echo ""
echo "Type System (OpenAPI-First):"
echo "  1. Zod schemas with OpenAPI metadata (src/schemas/auth.schema.ts)"
echo "  2. createRoute() definitions linking schemas to endpoints"
echo "  3. RouteHandler<typeof route> functions (src/handlers/*.ts)"
echo "  4. OpenAPIHono app auto-validates and generates spec at runtime"
echo "  5. Frontend SDK auto-generated from /openapi.json endpoint"
echo ""
echo "Key Benefits:"
echo "  ✓ Zero type drift between validation, types, and API docs"
echo "  ✓ Automatic request/response validation via middleware"
echo "  ✓ Full type safety from backend to frontend"
echo "  ✓ 89% reduction in manual type definitions"
echo ""
echo "See docs/TYPE_SYSTEM.md for complete architecture documentation"
echo ""
...
echo "    Fetch OpenAPI:   curl http://localhost:8787/openapi.json > openapi.json"
echo ""
echo "  Frontend:"
echo "    Start dev:       cd demo-app && pnpm run dev"
echo "    Build:           cd demo-app && pnpm run build"
echo "    Generate SDK:    cd demo-app && pnpm run generate:sdk"
echo "    Type check:      cd demo-app && pnpm run build.types"
echo ""
echo "  Infrastructure:"
...
echo "    Destroy infra:   cd infrastructure && pulumi destroy"
echo ""
echo "  Database:"
echo "    Cleanup DB:      ./scripts/database/cleanup-db.sh"
echo "    Delete user:     ./scripts/database/delete-user.sh <email>"
echo ""
echo "  Testing:"
echo "    Test backend:    ./scripts/testing/test-registration-backend.sh"
echo "    Test email:      ./scripts/testing/test-email.sh"
echo ""
